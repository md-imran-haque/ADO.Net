...
namespace HR_Management
{
    public partial class frmCity : Form
    {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            con.Open();
            string exist = "";
            SqlCommand cm = new SqlCommand("SELECT * FROM tblCity WHERE City='" + txtCity.Text + "' ", con);
            SqlDataReader dr = cm.ExecuteReader();
            if (dr.Read())
            {
                exist = dr["City"].ToString();
            }
            con.Close();
            ...
            else if (txtCity.Text.ToLower() == exist.ToLower())
            {
                MessageBox.Show("City " + txtCity.Text + " Already Exist!!!");
            }
            else
            {
                con.Open();
